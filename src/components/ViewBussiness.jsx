...
function ManageBusinesses() {
    ...
    useEffect(() => {
        const fetchBusinesses = async () => {
            try {
                const res = await axios.get('https://brsbackendserver.onrender.com/api/businesses');
    ...
    const handleUpdateBusiness = async (updatedBusiness) => {
        try {
            const res = await axios.put(`https://brsbackendserver.onrender.com/api/businesses/${updatedBusiness._id}`, updatedBusiness, {
                headers: {
    ...
    const handleDeleteBusiness = async (businessId) => {
        ...
        if (confirmed) {
            try {
                await axios.delete(`https://brsbackendserver.onrender.com/api/businesses/${businessId}`);
                            ...
                            <tr key={business._id} className="hover:bg-gray-50">
                                ...
                                <td className="py-2 px-4 border-b">
                                    {business.documents.map((doc, index) => (
                                        <a key={index} href={`https://brsbackendserver.onrender.com/${doc}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                                            Document {index + 1}
                                        </a>
                                    ))}
                                </td>
